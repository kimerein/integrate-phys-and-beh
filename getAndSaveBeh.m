function Response=getAndSaveBeh(continuingAnalysisDir,settings)

% combine data
Response.dprimes=nan(length(continuingAnalysisDir),1);
for i=1:length(continuingAnalysisDir)
    sub_continuingAnalysisDir=continuingAnalysisDir(i);
    disp('Running combineExptPieces');
    [alltbt,metadata]=combineExptPieces_local(sub_continuingAnalysisDir,settings.nameOfCue,settings.cueDuration,settings.doRealign,settings);
    if isempty(alltbt)
        if i==1
            error('First cannot be empty getAndSaveBeh.m');
        else
            continue
        end
    end
    ...
    if i==1
        Response.cue=nan(length(continuingAnalysisDir),size(alltbt.cueZone_onVoff,2));
        Response.reaches=nan(length(continuingAnalysisDir),size(alltbt.cueZone_onVoff,2));
        Response.times_wrt_trial_start=nan(length(continuingAnalysisDir),size(alltbt.times_wrt_trial_start,2));
    end
    Response.cue(i,:)=nanmean(alltbt.cueZone_onVoff,1);
    Response.reaches(i,:)=nanmean(alltbt.all_reachBatch,1);
    Response.dprimes(i)=nanmean(nanmean(alltbt.dprimes,2),1);
    Response.times_wrt_trial_start(i,:)=nanmean(alltbt.times_wrt_trial_start,1);
end

end

function [alltbt,allmetadata]=combineExptPieces_local(expt_dir,useAsCue,cueDuration,doRealign,settings)
...
for i=1:length(expt_dir)
    thisname=expt_dir{i};
    thisisdir=1;
    if ~isempty(regexp(thisname,'tbt')) && thisisdir==1
...
%         a=load([expt_dir '\' thisname '\tbt_resampled.mat']);
        if exist([thisname '\beh2_tbt.mat'], 'file')==2
            a=load([thisname '\beh2_tbt.mat']);
            tbt{j}=a.beh2_tbt;
        elseif exist([thisname '\behavior_tbt.mat'], 'file')==2
            a=load([thisname '\behavior_tbt.mat']);
            tbt{j}=a.behavior_tbt;
        else
            continue;
        end
