...
setForUn=settingsForStriatumPhotometryPlots;
...
if length(whichSess)>1
    ...
    ResponseCued=getAndSaveResponse(dd_more,whichUnitsToGrab,settingsForStriatumPhotometryPlots,[]);
...
if strcmp(doOrSaveGLM,'save')
    ...
    saveTo=[dd{whichSess} sep 'forPhotoglm' saveToAppend];
    if ~exist(saveTo, 'dir')
       mkdir(saveTo);
    end
...
strset=settingsForStriatumPhotometryPlots;
...
for j=1:length(dd)
    ...
    for i=3:length(ls)
        a=[];
        if strcmp(ls(i).name,'behavior_tbt.mat')
            a=load([ls(i).folder sep ls(i).name]);
            beh2_tbt=a.behavior_tbt;
        elseif strcmp(ls(i).name,'photometry_tbt.mat')
            a=load([ls(i).folder sep ls(i).name]);
            phys_tbt=a.photometry_tbt;
        else
            continue
        end
    end
    ...
    for i=1:length(getEventsFromPhysTbt)
        if ~isfield(phys_tbt,'opto') && strcmp('opto',getEventsFromPhysTbt{i})
            temp=zeros(size(phys_tbt.cue));
        else
