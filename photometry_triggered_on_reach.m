...
if isfield(alltbt1,'threwOutCuesAtFront')
    temp=alltbt.from_first_video(:,1);
    temp=temp(alltbt1.threwOutCuesAtFront+1:end);
else
    temp=alltbt.from_first_video(:,1);
end
...
if length(a)>1
    ...
    if isfield(alltbt2,'threwOutCuesAtFront')
        temp=alltbt.from_second_video(:,1);
        temp=temp(alltbt2.threwOutCuesAtFront+1:end);
    else
        temp=alltbt.from_second_video(:,1);
    end
    ...
    if isfield(totalalignment,'from_third_video')
        ...
        if isfield(alltbt3,'threwOutCuesAtFront')
            temp=alltbt.from_third_video(:,1);
            temp=temp(alltbt3.threwOutCuesAtFront+1:end);
        else
            temp=alltbt.from_third_video(:,1);
        end
...
if size(temp(fromCurrVid==1,:),1)~=length(cue_mapping_data_distract)
    n=input('Number of cues in data does not match number of cues in movie. Could be a problem. To throw out n distractors at end of movie (blue), enter positive number (negative number for front of movie). Or quit. n: ');
    if n>0
    ...
    elseif n<0
        n=abs(n);
        % throw out n distractors at beginning of video
        f=fieldnames(alltbt);
        for i=1:length(f)
            temp=alltbt.(f{i});
            temp=temp(n+1:end,:);
