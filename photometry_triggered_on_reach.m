...
temp=totalalignment.timesfromarduino(2:end)./1000-totalalignment.timesfromarduino(1:end-1)./1000;
temp=temp(temp~=0);
...
[discardedPhotoFrames_time,frontShift_time,scaleBy,movie_LED,movie_times,scaleMovieTimes,addToMovieTimes]=alignDistractors(totalalignment.movie_distractor(totalalignment.from_first_video==1),data.distractor,distract_thresh_movie,distract_thresh_photometry,totalalignment.timesfromarduino(totalalignment.from_first_video==1),data.distractor_times,settings,[],[]);
alltbt1=scaleMovTimes(alltbt,scaleMovieTimes,addToMovieTimes,alltbt.from_first_video(:,1));
[tbt_data_vid1,shifted_data_vid1,alltbt1]=shiftPhotometryToBehavior(data,discardedPhotoFrames_time,frontShift_time,movie_LED,movie_times,totalalignment.(useCue),totalalignment,alltbt1,minTimeBetweenCues,totalalignment.from_first_video==1,alltbt.from_first_video(:,1));
if isfield(alltbt1,'threwOutCuesAtEnd')
    temp=alltbt.from_first_video(:,1);
    temp=temp(1:end-alltbt1.threwOutCuesAtEnd);
else
    temp=alltbt.from_first_video(:,1);
end
alltbt1=selectRows(alltbt1,temp);
...
[tbt_data_vid2,shifted_data_vid2,alltbt2]=shiftPhotometryToBehavior(data,discardedPhotoFrames_time,frontShift_time,movie_LED,movie_times,totalalignment.(useCue),totalalignment,alltbt2,minTimeBetweenCues,totalalignment.from_second_video==1,alltbt.from_second_video(:,1));
if isfield(alltbt2,'threwOutCuesAtEnd')
    temp=alltbt.from_second_video(:,1);
    temp=temp(1:end-alltbt2.threwOutCuesAtEnd);
else
    temp=alltbt.from_second_video(:,1);
end
alltbt2=selectRows(alltbt2,temp);
...
if isfield(totalalignment,'from_third_video')
    ...
    [tbt_data_vid3,shifted_data_vid3,alltbt3]=shiftPhotometryToBehavior(data,discardedPhotoFrames_time,frontShift_time,movie_LED,movie_times,totalalignment.(useCue),totalalignment,alltbt3,minTimeBetweenCues,totalalignment.from_third_video==1,alltbt.from_third_video(:,1));
    if isfield(alltbt3,'threwOutCuesAtEnd')
        temp=alltbt.from_third_video(:,1);
        temp=temp(1:end-alltbt3.threwOutCuesAtEnd);
    else
        temp=alltbt.from_third_video(:,1);
    end
    alltbt3=selectRows(alltbt3,temp);
end
...
f=fieldnames(alltbt);
for i=1:length(f)
    if strcmp(f{i},'threwOutCuesAtEnd')
        continue
    end
    temp=alltbt.(f{i});
...
function [tbt_data,shifted_data,alltbt]=shiftPhotometryToBehavior(data,discardedPhotoFrames_time,frontShift_time,movie_LED,movie_times,movie_cue,totalalignment,alltbt,minTimeBetweenCues,fromCurrVid,fromCurrVid_tbt)
...
if size(temp(fromCurrVid==1,:),1)~=length(cue_mapping_data_distract)
    n=input('Number of cues in data does not match number of cues in movie. Could be a problem. Either throw out n distractors at end of movie (blue), or quit. n: ');
    % throw out n distractors at end of video
    f=fieldnames(alltbt);
    for i=1:length(f)
        temp=alltbt.(f{i});
        temp=temp(1:end-n,:);
...
function [discardedPhotoFrames_time,frontShift_time,scaleBy,movie_LED,movie_times,scaleMovieTimes,addToMovieTimes]=alignDistractors(movie_distract,photo_distract,distract_thresh_movie,distract_thresh_photometry,movie_times,photo_times,settings,isInBackHalf,fractionThrough)
