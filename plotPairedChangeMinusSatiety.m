...
curr_m(vec1_trial1(2,:)==0 & vec1_trial1(1,:)==0)=0;
actual_y_change=vec1(2,:);
expected_y_change=curr_m.*vec1(1,:);
expected_y_change(vec1(1,:)==0)=0;
actual_x_change=vec1(1,:);
expected_x_change=actual_y_change./curr_m;
expected_x_change(actual_y_change==0 & curr_m==0)=0;
expected_x_change(actual_y_change==0)=0;
...
for i=1:length(actual_y_change)
    ...
    % Project
    projected_to_orth(i)=actual_x_change(i)*V_orth(1)+actual_y_change(i)*V_orth(2);
    projected_to_par(i)=actual_x_change(i)*V_par(1)+actual_y_change(i)*V_par(2);
end
projected_to_orth(actual_x_change==0 | actual_y_change==0 | curr_m==0 | isinf(curr_m))=nan;
projected_to_par(actual_x_change==0 | actual_y_change==0 | curr_m==0 | isinf(curr_m))=nan;
    
% Nan infinities, don't plot or consider
x_comps_change(isinf(x_comps_change))=nan;
y_comps_change(isinf(y_comps_change))=nan;
...
howmany=length(tempx);
line([nanmean(tempx)-nanstd(tempx,[],2)./sqrt(howmany) nanmean(tempx)+nanstd(tempx,[],2)./sqrt(howmany)],...
     [nanmean(tempy) nanmean(tempy)],'Color','b','LineWidth',2);
line([nanmean(tempx) nanmean(tempx)],...
     [nanmean(tempy)-nanstd(tempy,[],2)./sqrt(howmany) nanmean(tempy)+nanstd(tempy,[],2)./sqrt(howmany)],'Color','b','LineWidth',2);
...
% Projections
% xlab='Deviation projected onto proportionality line (1/sec)';
% ylab='Deviation projected onto orthogonal (1/sec)';
% plotVersus(fracsThroughSess,projected_to_par,projected_to_orth,xlab,ylab,0.01);
% daspect([1 1 1]);
% tempx=projected_to_par(projected_to_orth>0 & ~isnan(projected_to_orth) & ~isnan(projected_to_par));
% tempy=projected_to_orth(projected_to_orth>0 & ~isnan(projected_to_orth) & ~isnan(projected_to_par));
% howmany=length(tempx);
% line([nanmean(tempx)-nanstd(tempx,[],2)./sqrt(howmany) nanmean(tempx)+nanstd(tempx,[],2)./sqrt(howmany)],...
%      [nanmean(tempy) nanmean(tempy)],'Color','b','LineWidth',2);
% line([nanmean(tempx) nanmean(tempx)],...
...
% xlabel('Deviation projected orthogonal');
% ylabel('Counts');
...
keeps=~isnan(x_comps_change) & ~isnan(y_comps_change);
xlim([min(x_comps_change(keeps),[],2,'omitnan') max(x_comps_change(keeps),[],2,'omitnan')]);
ylim([min(y_comps_change(keeps),[],2,'omitnan') max(y_comps_change(keeps),[],2,'omitnan')]);
line([0 0],[min(y_comps_change(keeps),[],2,'omitnan') max(y_comps_change(keeps),[],2,'omitnan')],'Color','k');
line([min(x_comps_change(keeps),[],2,'omitnan') max(x_comps_change(keeps),[],2,'omitnan')],[0 0],'Color','k');
xlabel(xlab);
ylabel(ylab);
