function [dataset,correctedDistributions]=buildReachingRTModel(varargin)
...
%       dataset is the raw data set of reaction time changes between trial pairs
%
%       correctedDistributions are the reaction time changes, given a resampling
%       of the reaction time distribution in trial 1 (first in pair) to
%       match reaction time distribution across all trials

[alltbt,trialTypes,metadata]=backwardTime(alltbt,trialTypes,metadata); % will flip order of trials only if RTanalysis_settings.reverse_trial_order is true
...
function [alltbt,trialTypes,metadata]=backwardTime(alltbt,trialTypes,metadata)

temp=RTanalysis_settings();
reverse_time=temp.reverse_trial_order;
...
for i=1:length(nInSequence)
    ...
    for j=1:nRepsForBootstrap
        ...
        if doCorrectInputDistribution==false && j==1
            % take all trials, will need later
            rawR_cond1_trial1=[rawR_cond1_trial1; rawReaching_cond1_trial1];
            rawR_cond1_triali=[rawR_cond1_triali; rawReaching_cond1_triali];
            rawR_cond2_trial1=[rawR_cond2_trial1; rawReaching_cond2_trial1];
            rawR_cond2_triali=[rawR_cond2_triali; rawReaching_cond2_triali];
        elseif doCorrectInputDistribution==true
