...
for i=1:length(spikeFieldsNames)
    ...
    %sig=fillInOtherSignalsSkipCue(nextSignalTbt,nextSignalTimes,signal_times_wrt_thisTrialStart,signal_which_trial);
    if ~isempty(regexp(spikeFieldsNames{i},'unit'))
        disp(['Filling in ' spikeFieldsNames{i}]);
        signalSpikeFields.resampPhysTimepoints=resampledPhysTimepoints;
        if didAUnit==false
            [sig,fs,mi_inds]=fillInOtherSignalsPhysiology(nextSignalTbt,nextSignalTimes,resampledSignalCue,signal_cue_times_wrt_thisTrialCueStart,signal_which_trial,signalSpikeFields,spikeFieldsNames{i},signal_cue,signal_cue_times,[],[]);
            didAUnit=true;
        else
            sig=fillInOtherSignalsPhysiology(nextSignalTbt,nextSignalTimes,resampledSignalCue,signal_cue_times_wrt_thisTrialCueStart,signal_which_trial,signalSpikeFields,spikeFieldsNames{i},signal_cue,signal_cue_times,fs,mi_inds);
        end
    else
        sig=fillInOtherSignals(nextSignalTbt,nextSignalTimes,nextSignalCue,signal_cue_times_wrt_thisTrialCueStart,signal_which_trial);
    end
...
function [sig,fs,mi_inds]=fillInOtherSignalsPhysiology(nextSignalTbt,nextSignalTimes,nextSignalCue,signal_cue_times_wrt_thisTrialCueStart,signal_which_trial,photo_tbt,whichCh,signal_cue,signal_cue_times,fs,mi_inds)
...
cue_data=[];
photodata=photo_tbt.(whichCh);
save_fs=cell(1,size(photo_tbt.resampPhysTimepoints,1));
...
if useAlignment==false
    ...
    if isempty(mi_inds)
        mi_inds=nan(1,length(signal_cue_times));
        for i=1:length(signal_cue_times)
            [~,mi]=nanmin(abs(green_times-signal_cue_times(i)));
            sig(i)=green_ch_data(mi);
            mi_inds(i)=mi;
