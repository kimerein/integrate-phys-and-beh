function LDA_analysis(whichSess,downSampBy,takeNPointsAfterEvent,nPointsBeforeEvent,response_to_plot1,response_to_plot2,dd)
...
for i=1:length(allTri)
    ...
    for j=1:length(us)
        ...
        if sum(ResponseCued.fromWhichTrial==currTri & ResponseCued.fromWhichUnit==currU,'all','omitnan')==0
            ...
            mergedData(i,(j-1)*(takeNPointsAfterEvent+nPointsBeforeEvent)+1:j*(takeNPointsAfterEvent+nPointsBeforeEvent))=ResponseCued.unitbyunit_y(ResponseCued.fromWhichTrial==currTri & ResponseCued.fromWhichUnit==currU,zeroind-nPointsBeforeEvent:zeroind+takeNPointsAfterEvent-1);
        end
    end
end
...
for i=1:length(allTri)
    ...
    for j=1:length(us)
        ...
        if sum(ResponseUncued.fromWhichTrial==currTri & ResponseUncued.fromWhichUnit==currU,'all','omitnan')==0
            ...
            mergedData(i,(j-1)*(takeNPointsAfterEvent+nPointsBeforeEvent)+1:j*(takeNPointsAfterEvent+nPointsBeforeEvent))=ResponseUncued.unitbyunit_y(ResponseUncued.fromWhichTrial==currTri & ResponseUncued.fromWhichUnit==currU,zeroind-nPointsBeforeEvent:zeroind+takeNPointsAfterEvent-1);
        end
    end
end
% take only trials where event occurred
