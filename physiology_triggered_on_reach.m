...
%     settings.minlagForInitialAlign=422; % new values
%     settings.maxlagForInitialAlign=424;
...
close all;
ds=1000; % use this downsample factor for the fields in downsampfields
...
%         isInBackHalf=[];
%         fractionThru=[];
        [discardedPhotoFrames_time,frontShift_time,scaleBy,movie_LED,movie_times,scaleMovieTimes,addToMovieTimes,padPhotoTimesAtFront]=alignDistractors(totalalignment.movie_distractor(totalalignment.from_fourth_video==1),data.distractor,distract_thresh_movie,distract_thresh_photometry,totalalignment.timesfromarduino(totalalignment.from_fourth_video==1),data.distractor_times,settings,isInBackHalf,fractionThru);
        alltbt4=scaleMovTimes(alltbt,scaleMovieTimes,addToMovieTimes,alltbt.from_fourth_video(:,1));
        [tbt_data_vid4,shifted_data_vid4,alltbt4]=shiftPhotometryToBehavior(data,discardedPhotoFrames_time,frontShift_time,movie_LED,movie_times,totalalignment.(useCue),totalalignment,alltbt4,minTimeBetweenCues,totalalignment.from_fourth_video==1,alltbt.from_fourth_video(:,1),padPhotoTimesAtFront);
        fromvid=alltbt.from_fourth_video(:,1);
        if isfield(alltbt4,'threwOutCuesAtEnd')
            fromvid(end-alltbt4.threwOutCuesAtEnd+1:end)=0;
        end
        if isfield(alltbt4,'threwOutCuesAtFront')
...
if length(a)>1
    ...
    if isfield(totalalignment,'from_fourth_video')
        [photometry_tbt]=concatTbt(photometry_tbt,tbt_data_vid4);
        [behavior_tbt]=concatTbt(behavior_tbt,alltbt4);
    end
else
...
elseif ~isfield(alignment1,'from_fourth_video')
    outalignment.from_first_video=zeros(size(outalignment.(f{1})));
    outalignment.from_first_video(1:nansum(alignment1.from_first_video))=alignment1.from_first_video(1:nansum(alignment1.from_first_video));
    outalignment.from_second_video=zeros(size(outalignment.(f{1})));
    outalignment.from_second_video(nansum(alignment1.from_first_video)+1:nansum(alignment1.from_first_video)+nansum(alignment1.from_second_video))=alignment1.from_second_video(nansum(alignment1.from_first_video)+1:nansum(alignment1.from_first_video)+nansum(alignment1.from_second_video));
    outalignment.from_third_video=zeros(size(outalignment.(f{1})));
    outalignment.from_third_video(nansum(alignment1.from_second_video)+1:nansum(alignment1.from_second_video)+nansum(alignment1.from_third_video))=alignment1.from_third_video(nansum(alignment1.from_second_video)+1:nansum(alignment1.from_second_video)+nansum(alignment1.from_third_video));
    outalignment.from_fourth_video=zeros(size(outalignment.(f{1})));
    outalignment.from_fourth_video(length(alignment1.(f{1}))+1:end)=1;
end
...
figure(); plot(photo_LED,'Color','k'); hold on; plot(movie_LED,'Color','r'); ylim([0 1.1]);
