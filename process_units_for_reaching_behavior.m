...
figure(); imagesc(smoothMatrix(nama_all_glm_coef(idx_from_glm==3 & ~any(abs(nama_all_glm_coef)>10,2),214:end),10)); figure(); imagesc(smoothMatrix(nama_all_glm_coef(idx_from_glm==1 & ~any(abs(nama_all_glm_coef)>10,2),214:end),10));
...
nama_all_glm_coef=nama_all_glm_coef(orderingSuccVFail,:); nama_all_glm_coef(nama_all_glm_coef<0)=0;
succ_nama_all_glm_coef=smoothMatrix(nama_all_glm_coef(:,71*3+1:71*3+71),4); cueXsucc_nama_all_glm_coef=smoothMatrix(nama_all_glm_coef(:,71*6+1:71*6+71),4);
fail_nama_all_glm_coef=smoothMatrix(nama_all_glm_coef(:,71*4+1:71*4+71)+nama_all_glm_coef(:,71*5+1:71*5+71),4); cueXfail_nama_all_glm_coef=smoothMatrix(nama_all_glm_coef(:,71*7+1:71*7+71)+nama_all_glm_coef(:,71*8+1:71*8+71),4);
load('Z:\MICROSCOPE\Kim\Physiology Final Data Sets\GLM\python glm training set\idx_from_glm.mat'); 
figure(); scatter(Y(idx_from_glm(~isnan(idx_from_glm))==1,1),Y(idx_from_glm(~isnan(idx_from_glm))==1,2),[],'k'); hold on; scatter(Y(idx_from_glm(~isnan(idx_from_glm))==2,1),Y(idx_from_glm(~isnan(idx_from_glm))==2,2),[],'r');
[n,x]=histcounts(Y(idx_from_glm(~isnan(idx_from_glm))==1,2),-25-0.5:1:25+0.5); [n,x]=cityscape_hist(n,x); figure(); plot(x,n,'Color','k');
[n,x]=histcounts(Y(idx_from_glm(~isnan(idx_from_glm))==2,2),-25-0.5:1:25+0.5); [n,x]=cityscape_hist(n,x); hold on; plot(x,n,'Color','r');
idx_from_glm=idx_from_glm(orderingSuccVFail);
figure(); imagesc(smoothMatrix(nama_all_glm_coef(idx_from_glm==1,214:end),4)); figure(); imagesc(smoothMatrix(nama_all_glm_coef(idx_from_glm==2,214:end),4));
