...
mixedLoading=nan(length(dd),whatRank);
for i=1:length(dd)
...
for i=1:length(loss)
    ...
    hold on;
    plot(1:size(sumByFac,2),sumByFac(i,:),'LineWidth',1,'Color',c);
end
...
pairwiseDiffs=nan(size(neuron_fac,1),size(neuron_fac,2)*size(neuron_fac,2));
