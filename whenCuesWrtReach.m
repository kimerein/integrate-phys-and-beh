function alltimesfromcue=whenCuesWrtReach(data_loc_array,whichreach)

alltimesfromcue=[];
for i=1:size(data_loc_array,1)
    if exist([data_loc_array{i,6} '\beh2_tbt.mat'],'file')
        a=load([data_loc_array{i,6} '\beh2_tbt.mat']);
        [timesfromcue,indsfromcue]=forday_whenCuesWrtReach(a.beh2_tbt,whichreach);
...
function [timesfromcue,indsfromcue]=forday_whenCuesWrtReach(beh2_tbt,whichreach)
