...
disp(['Progress through labeling in this order:' newline ...
      ...
      'Double click image, repeat to label multiple points' newline ...
      'Be sure to label many different points spanning the 3D space where the paw might go' newline ...
      'Once all points defined on image, close figure (X)' newline ...
      'Then press Enter at Matlab command line']);
...
disp('Choose frame where pellet stopped in front of mouse.');
...
disp('Choose frame where paw fully outstretched in front of mouse.');
fig=implay(allframes,30);
fig.Parent.Position=[100 100 800 800];
pause;
if ~isempty(regexp(version,'2017b','once')) || ~isempty(regexp(version,'2018b','once')) ||  ~isempty(regexp(version,'2020b','once')) || ~isempty(regexp(version,'2021a','once'))  || ~isempty(regexp(version,'2021b','once'))
...
side_image_points=flipImagePoints(side_image_points,size(frame,2),size(frame,1),'horizontal'); % note that user clicks on img are flipped x and y
under_image_points=flipImagePoints(under_image_points,size(frame,2),size(frame,1),'horizontal');
mirror_vs(1:2)=flipImagePoints(mirror_vs(1:2),size(frame,2),size(frame,1),'horizontal');
mirror_vs(3:4)=flipImagePoints(mirror_vs(3:4),size(frame,2),size(frame,1),'horizontal');
